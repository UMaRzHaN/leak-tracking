        ...
        output: {
          ...
          codeSplitting: {
            groups: [
              ...
              // Плагины, которые нужны только по действию человека: камера,
              // сканер штрихкодов, распознавание речи, публикация файла.
              // Пока они лежали в одном чанке с ядром и файловой системой,
              // их затаскивал на первый экран любой репозиторий — тому нужен
              // @capacitor/filesystem, а чанк общий. Группа объявлена раньше
              // общей: совпадения проверяются по порядку.
              // Ядро и файловая система нужны до первой отрисовки: их тянет
              // любой репозиторий. Названы явно, а не «всё остальное», —
              // иначе реализация ядра уезжала в соседнюю группу вместе с
              // плагинами, которые от неё зависят, и утаскивала их обратно на
              // первый экран.
              {
                name: "vendor-capacitor",
                test: /[\\/]node_modules[\\/]@capacitor[\\/](core|filesystem|geolocation)[\\/]/,
              },
              // Всё, что нужно только по действию человека: камера, публикация
              // файла, сканер штрихкодов, распознавание речи. Импорты этих
              // плагинов в коде — динамические.
              {
                name: "vendor-capacitor-ondemand",
                test: /[\\/]node_modules[\\/](@capacitor[\\/](camera|share)|@capacitor-community[\\/]|@capacitor-mlkit[\\/])/,
              },
              {
                name: "vendor-excel",
