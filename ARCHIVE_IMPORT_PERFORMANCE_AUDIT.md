...
Целевая структура `services/` описана в `CONTRIBUTING.md` («Subsystem layout under `services/`»); переезд по таблице из раздела 8 этого документа уже выполнен. Ниже — конкретные строки кода, из-за которых импорт архивов работает медленнее, чем должен, и порядок исправлений.
...
| #   | Проблема                                                    | Файл                                             | Приоритет           | Эффект                             | Статус                  |
| --- | ----------------------------------------------------------- | ------------------------------------------------ | ------------------- | ---------------------------------- | ----------------------- |
| 1   | Двойная распаковка архива (verify + реальное использование) | `utils/importLimits.js`                          | P0                  | Наибольший                         | ⏳ открыт               |
| 2   | `hydrateZipPhotos` без параллелизма                         | `services/excelImport/photoPipeline.js`          | P0                  | Средний-высокий                    | ✅ сделано              |
| 3   | Побайтовое сравнение фото вместо хеша                       | `services/excelImport/photoPipeline.js`          | P0                  | Средний                            | ✅ сделано              |
| 4   | Нет `yield` в парсинге строк Excel                          | `services/excelImportService.js`                 | P0                  | Средний (отзывчивость UI)          | ✅ сделано              |
| 5   | Захардкоженный параллелизм = 3 везде                        | `photoPipeline.js`, `projectBackup/constants.js` | P1 (нужен бенчмарк) | Средний                            | ⏳ открыт (нужен замер) |
| 6   | Импорт без Web Worker                                       | весь путь импорта                                | P1                  | Наибольший для восприятия скорости | ⏳ открыт               |
| 7   | CRC32 побайтово при экспорте                                | `services/zipStoreStream.js`                     | P2                  | Низкий (не импорт)                 | ⏳ открыт               |

Пункты 1-4 не меняют формат архива и не требуют миграций — их можно сделать отдельными небольшими PR без риска для обратной совместимости, в соответствии с ограничениями совместимости данных из `CONTRIBUTING.md` («Data compatibility constraints»).

### Почему пункт 1 остался открытым

`verifyArchiveLimits()` — это не только оптимизируемый лишний проход, но и текущая защита от zip-бомб: она проверяет **фактический** распакованный размер, а не заявленный в ZIP-заголовке. Оба предложенных в разделе 1 варианта ослабляют эту гарантию (первый — до проверки «по ходу чтения», второй — до доверия заголовку), поэтому это изменение границы безопасности, а не чистая оптимизация. Делать его нужно вместе с явным решением, какой уровень защиты от zip-бомб считается достаточным, и с тестом на архив с заниженным `uncompressedSize`.
...
Целевая структура зафиксирована в `CONTRIBUTING.md` («Subsystem layout under `services/`»). Конкретное распределение файлов из корня:
...
**Как переносить, не ломая проект — переезд файлов не смешивается с изменением поведения:**
