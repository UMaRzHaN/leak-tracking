    ...
    synchronized ProjectPage loadProjectPage(
        ...
        try (
            Cursor cursor = database.query(
                ...
                // Форма с запятой верна на любой версии, поэтому осталась и
                // после подъёма minSdk до 33: платформа со словом OFFSET
                // теперь справится, но менять работающее ради этого незачем.
                offset + "," + limit
            )
        ) {
            boolean first = true;
