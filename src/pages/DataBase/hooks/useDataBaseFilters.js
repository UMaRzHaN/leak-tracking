...
const SEARCH_KEYS = ["leak_id", "object", "component", "location", "field", "leak_description"];
...
export function useDataBaseFilters({ data, coords }) {
  ...
  const displayed = useMemo(() => {
    ...
    const applySearch = (list) =>
      q ? list.filter((l) => SEARCH_KEYS.some((k) => l[k]?.toLowerCase().includes(q))) : list;

    const applyPriority = (list) =>
      priorityFilter !== ALL ? list.filter((l) => (l.priority ?? null) === priorityFilter) : list;

    if (statusFilter === NEARBY) {
      const base = hasGps ? filterNearbyLeaks(data, coords.lat, coords.lng, NEARBY_RADIUS_M) : [];
      return applySearch(applyPriority(base));
    }

    let list = [...data].sort((a, b) => sortAsc ? a.id - b.id : b.id - a.id);
    if (statusFilter !== ALL) list = list.filter((l) => (l.status ?? STATUS.OPEN) === statusFilter);
  ...
  return {
    ...
    sortAsc, toggleSort: () => setSortAsc((v) => !v),
    hasGps,
    displayed,
    counts,
  };
