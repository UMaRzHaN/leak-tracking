/**
 * Жизненный цикл записи об утечке.
 *
 * Значения — те же три, что объявлены типом `LeakStatus`: описание домена и
 * константы, которыми пользуется код, обязаны совпадать, и приведение ниже это
 * обеспечивает — четвёртый статус здесь не пройдёт проверку типов.
 *
 * @type {Record<"OPEN"|"IN_PROGRESS"|"RESOLVED", import("@/types/domain").LeakStatus>}
 */
export const STATUS = {
  OPEN: "open",
