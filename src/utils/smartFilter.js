...
const ABBREV_MAP = {
  ...
  "ндп":  "нефтедобывающее управление",
...
function optionMatchesAbbrev(normalizedOption, abbrev, expandedForm) {
  ...
  const token = abbrev + " ";
  return normalizedOption.startsWith(token) || normalizedOption.includes(" " + token);
}
...
function calculateRelevance(query, option) {
  ...
  if (hasAbbreviation && hasNumbers) {
    ...
    if (optionMatchesAbbrev(o, abbrevWord, expanded)) {
      ...
      if (nums.dn && nums.pn && optionNums.dn === nums.dn && optionNums.pn === nums.pn) return 100;
...
export function smartFilter(query, options, minScore = 60) {
  ...
  for (const abbr of Object.keys(ABBREV_MAP).sort((a, b) => b.length - a.length)) {
    if (q.includes(abbr)) {
      foundAbbrev = abbr;
