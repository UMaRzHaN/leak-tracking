...
const EXCLUDED_KEYS = new Set([
  ...
  "history",
  "monitoringRecords",
  // Списки сводятся объединением по номеру записи, а не выбором победителя по
  // версии поля. Версия на них не только бесполезна — она сделала бы из двух
  // независимо дописанных событий одно.
  //
  // Рядом стояло `monitoringTombstones`. Механизма за этим именем не было
  // нигде: ни записи, ни чтения, ни удаления обхода в интерфейсе — и не могло
  // быть, потому что история в приложении не правится осознанно. Имя в списке
  // исключений выглядело договорённостью и заставляло искать несуществующее;
  // когда удаление записи обхода понадобится, надгробия заводятся вместе с ним.
  "events",
