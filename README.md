...
📖 **[Руководство пользователя со скриншотами](docs/MANUAL.md)** · 📱 **[Отдельное руководство по Android](docs/manual/android/manual.html)** ([как собрать PDF](docs/MANUAL.md#руководство-по-android))
...
`npm run check:install-scripts` показывает, чьи install-скрипты ждут решения:
разрешены ровно четыре пакета, список — в `allowScripts` файла `package.json`.
...
npm run android:check  # Сборка + cap:sync + установка release-двойника на устройство
```
...
npm run check:android-r8         # R8 не выкинул из релиза runtime Capacitor
npm run artifacts:checksums:web  # Чек-суммы артефактов (есть :android, :source, :e2e)
npm run release:evidence:web     # Манифест верификации по точной SHA (те же четыре охвата)
```

Охваты у последних двух одни и те же и соответствуют задачам CI: `web`,
`android`, `source`, `e2e`. Каждая задача пишет манифест и чек-суммы своего
охвата, поэтому по артефактам запуска видно, что именно проверялось.
...
Версия релиза живёт в `ANDROID_VERSION_NAME`, а не в `package.json`. Поле
`version` там осталось на `1.0.0` с самого начала, пока наружу уходили 1.0.1,
1.0.2 и 1.0.3; поднимать его — значит завести вторую нумерацию, которая
разойдётся с первой.

`ANDROID_VERSION_CODE` обязан расти с каждой публикацией. Приложение
распространяется вне магазинов, и никто, кроме самих артефактов, прошлый номер
не помнит — ошибиться вниз означает собрать APK, который не встанет
обновлением на телефоны в поле. Где посмотреть предыдущий:

```bash
# номер последней собранной здесь сборки
cat android/app/build/outputs/apk/release/output-metadata.json

# то же плюс коммит, чистота дерева и чек-суммы APK
cat .artifacts/release-verification-android.json
```

Надёжнее артефактов — теги: они уезжают на удалённый и переживают чистку
`android/build`.

```bash
git tag -l -n1        # какие релизы уже были
git describe --tags   # что собрано из текущего состояния
```

Собранный релиз помечается аннотированным тегом `vX.Y.Z` на том коммите, с
которого он собран, — и только после того, как APK существует и проверен.
Аннотация несёт то, чего нет в самом теге: чек-сумму APK и отпечаток подписи.

```bash
git tag -a v1.1.0 -m "Релиз 1.1.0 (versionCode 5)
sha256:  <чек-сумма app-release.apk>
подпись: <отпечаток из apksigner verify>" <коммит>
git push origin v1.1.0
```

Тег ставится на коммит с чистым деревом: `release-verification-android.json`
записывает `dirty` и список правок, и тег, указывающий на состояние, которого
не было в истории, ничего не удостоверяет.
...
`.github/workflows/ci.yml`, Node 22, семь задач. Первая — привратник: остальные
шесть ждут её и не запускаются, если она упала.

| Задача                    | Что делает                                                                                                                                                                                                                               |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `quality`                 | Чистота дерева → lint → format → три проверки типов (`typecheck`, `:strict`, `:tools`) → аудит зависимостей → бюджет сложности → тесты с покрытием → храповик покрытия → сборка → бюджет бандла → лицензии → SBOM → манифест и чек-суммы |
| `source-artifact`         | Собирает source-архив и проверяет его установкой с нуля                                                                                                                                                                                  |
| `performance`             | Бюджеты производительности на 10 000 записей в Chromium                                                                                                                                                                                  |
| `e2e`                     | Сценарии в Chromium плюс офлайн-прогон собранного приложения                                                                                                                                                                             |
| `cross-browser`           | Те же сценарии в Firefox и WebKit                                                                                                                                                                                                        |
| `android`                 | `gradlew test lintDebug assembleDebug lintRelease assembleRelease` и проверка, что R8 сохранил Capacitor                                                                                                                                 |
