...
утечек в каждой. Выбранная папка задаёт область работы: База, Карта и
Мониторинг показывают только утечки внутри неё. Это альтернатива перебору
фильтров, когда записей тысячи.

Область хранится не отдельно, а в тех же фильтрах локации, что и чекбоксы в
Базе (`mainLocationFilter`, `locationFilter`, `lastLocationFilter`), поэтому
две панели не могут разойтись, а выбор переживает перезагрузку и переключение
проекта. Если фильтр в Базе задан несколькими значениями, единого пути не
существует — шапка честно показывает «Выбрано несколько» вместо одного из них.
