...
- **Бюджет сложности ограничивает всё, а не список.** Раньше
  ...
  коммита. Тесты пока вне бюджета — там свои основания для длины.

  Исключение поднимали один раз: `seed100leaks.js` с 1404 до 1407, когда сид
  делали повторяемым. Правило «не занижать бюджеты ради прохода CI» этого не
  разрешает, и решение принято отдельно, а не по ходу: разрезать файл нельзя —
  он вставляется в страницу строкой и импортов не имеет, — а подрезать
  пояснения ради трёх строк значит спрятать строку вместо того, чтобы принять
  решение. Если такое понадобится снова, это повод резать файл, а не бюджет.
